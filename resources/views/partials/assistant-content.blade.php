...
<div class="flex min-h-0 flex-1 flex-col">
    ...
    <header class="flex h-16 shrink-0 items-center gap-3 border-b border-slate-950/10 px-4 dark:border-white/10 sm:px-5">
        ...
        <div class="min-w-0 flex-1">
            <h2 id="docent-assistant-title-reader" x-ref="assistantHeading" tabindex="-1" class="truncate text-base font-semibold text-slate-950 dark:text-white">{{ __('docent::ui.assistant.title') }}</h2>
            <p class="truncate text-sm text-slate-500 dark:text-slate-400">{{ __('docent::ui.assistant.subtitle') }}</p>
        </div>

        <button type="button" @click="toggleExpanded()" :aria-label="assistantExpanded ? @js(__('docent::ui.assistant.collapse')) : @js(__('docent::ui.assistant.expand'))"
        ...
        <button x-show="messages.length > 0" x-cloak type="button" @click="newConversation()" aria-label="{{ __('docent::ui.assistant.start_new_conversation') }}" title="{{ __('docent::ui.assistant.new_conversation') }}"
        ...
        <button type="button" @click="closeAssistant()" aria-label="{{ __('docent::ui.assistant.close') }}"
                class="relative inline-flex size-9 shrink-0 items-center justify-center rounded-md text-slate-500 hover:bg-slate-950/5 hover:text-slate-900 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--docent-accent)] dark:text-slate-400 dark:hover:bg-white/10 dark:hover:text-white">
            <span class="pointer-fine:hidden absolute left-1/2 top-1/2 size-[max(100%,3rem)] -translate-1/2" aria-hidden="true"></span>
    ...
    <div x-ref="assistantScroller" class="docent-scroll min-h-0 flex-1 overflow-y-auto" :aria-busy="asking">
        <div x-ref="assistantMessages" @click="copyCode($event)" class="mx-auto flex min-h-full w-full max-w-3xl flex-col gap-6 px-4 py-6 sm:px-6 sm:py-8" role="list" aria-label="{{ __('docent::ui.assistant.conversation') }}">
            <div x-show="messages.length === 0 && !asking" class="flex flex-1 flex-col items-center justify-center gap-3 py-12 text-center">
                ...
                <div class="flex flex-col gap-1">
                    <h3 class="text-balance text-lg font-semibold text-slate-950 dark:text-white">{{ __('docent::ui.assistant.empty_title') }}</h3>
                    <p class="mx-auto max-w-[36ch] text-pretty text-base text-slate-500 dark:text-slate-400 sm:text-sm">{{ __('docent::ui.assistant.empty_description') }}</p>
            ...
            <template x-for="message in messages" :key="message.id">
                <div role="listitem" class="min-w-0">
                    ...
                    <template x-if="message.role === 'assistant'">
                        <article class="min-w-0" :aria-label="message.status === 'streaming' ? @js(__('docent::ui.assistant.answering')) : @js(__('docent::ui.assistant.answer'))">
                            <div x-show="message.status === 'streaming' && message.content === ''" class="flex flex-col gap-3" aria-hidden="true">
                                <p class="text-base text-slate-500 dark:text-slate-400 sm:text-sm">{{ __('docent::ui.assistant.reading') }}</p>
                            ...
                            <div x-show="message.status === 'error'" class="flex flex-col gap-3 rounded-xl bg-rose-50 p-4 dark:bg-rose-950/30">
                                ...
                                <button type="button" @click="retry(message)"
                                    ...
                                    {{ __('docent::ui.assistant.try_again') }}
                                </button>
                            </div>
                            ...
                            <div x-show="message.status === 'complete' && message.html" class="mt-6 flex flex-col gap-4 border-t border-slate-950/10 pt-4 dark:border-white/10">
                                <div x-show="citedPages(message).length > 0" class="flex flex-col gap-2">
                                    <p class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ __('docent::ui.assistant.sources') }}</p>
                                ...
                                <div class="flex items-center gap-1">
                                    <button type="button" @click="copyAnswer(message)" :aria-label="message.copied ? @js(__('docent::ui.assistant.answer_copied')) : @js(__('docent::ui.assistant.copy_answer'))"
                                    ...
                                    <button x-show="message.id === messages[messages.length - 1]?.id" type="button" @click="regenerate(message)" aria-label="{{ __('docent::ui.assistant.regenerate_answer') }}"
                                            class="relative inline-flex size-9 items-center justify-center rounded-md text-slate-500 hover:bg-slate-950/5 hover:text-slate-900 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--docent-accent)] dark:text-slate-400 dark:hover:bg-white/10 dark:hover:text-white">
                                        <span class="pointer-fine:hidden absolute left-1/2 top-1/2 size-[max(100%,3rem)] -translate-1/2" aria-hidden="true"></span>
                                    ...
                                    <button type="button" @click="sendFeedback(message, 'up')" aria-label="{{ __('docent::ui.assistant.helpful_answer') }}" :aria-pressed="message.feedback === 'up'"
                                    ...
                                    <button type="button" @click="sendFeedback(message, 'down')" aria-label="{{ __('docent::ui.assistant.not_helpful_answer') }}" :aria-pressed="message.feedback === 'down'"
                                    ...
                                    <p x-show="message.copied || message.feedback" x-text="message.copied ? @js(__('docent::ui.assistant.copied_feedback')) : @js(__('docent::ui.assistant.thanks_for_feedback'))" class="ml-2 text-sm text-slate-500 dark:text-slate-400"></p>
                                </div>
                            </div>
                        </article>
                    </template>
    ...
    <footer class="shrink-0 border-t border-slate-950/10 bg-white p-3 dark:border-white/10 dark:bg-slate-950 sm:p-4">
        <form @submit.prevent="submit()" class="mx-auto max-w-3xl">
            <div class="relative rounded-2xl bg-white shadow-sm ring-1 ring-slate-950/10 focus-within:ring-2 focus-within:ring-[var(--docent-accent)] dark:bg-slate-900 dark:shadow-none dark:ring-white/10">
                <label for="docent-assistant-question-{{ $widgetAssistant ? 'widget' : 'reader' }}" class="sr-only">{{ __('docent::ui.assistant.ask_label') }}</label>
                <textarea id="docent-assistant-question-{{ $widgetAssistant ? 'widget' : 'reader' }}" name="question" x-ref="assistantComposer" x-model="composer"
                          ...
                          :disabled="asking" rows="2" maxlength="500" placeholder="{{ __('docent::ui.assistant.follow_up_placeholder') }}"
                ...
                <button x-show="!asking" type="submit" :disabled="composer.trim() === ''" aria-label="{{ __('docent::ui.assistant.ask_question') }}"
                ...
                <button x-show="asking" x-cloak type="button" @click="interrupt()" aria-label="{{ __('docent::ui.assistant.stop_generating') }}"

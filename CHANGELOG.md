...
- New opt-in `gated-link` check, reporting links from an ungated page to a gated one. `broken-link` validates that a target exists; this asks whether the readers of the linking page can open it. The failure surfaces late, since the author can see both pages and CI stays green — the only signal is a user with a narrower role reporting a link that goes nowhere. Deliberately narrow: only a source with no requirement at all provably reaches a wider audience than a gated target, so two differently-gated pages are never compared. A link nested inside a `:::can` or `:::audience` block is already gated in context and exempt, which doubles as the escape hatch for a deliberate "here's the page you'll need an administrator for". Enable with `'gated-link' => 'warning'` in `docent.check.rules`.
